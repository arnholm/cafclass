...
namespace cli {
	...
	protected:
		...
		std::string usage() const {
			std::stringstream ss { };
			ss <<  std::endl;
			ss << _general_help_text << std::endl;
			ss << "Parameters:" << std::endl;

			const size_t width = 12;

			for (const auto& command : _commands) {
				ss << "  " << command->command; // << "  " << std::setw(12) << std::left << command->alternative;

				if (command->required == true) {
                ss << "  " << std::setw(width) << std::left << "(required)" << std::setw(width) << " ";
				}
				else {
                ss << "  " << std::setw(width) << std::left << "(optional)";
                std::string def = command->print_value();
                std::stringstream ss2;
                if(def != "" && def != "[ ]") {
                   ss2 << std::left << ", def=" << def;
                }
                else {
                   ss2 << " ";
                }
                ss << std::setw(width) << ss2.str();
 				}
